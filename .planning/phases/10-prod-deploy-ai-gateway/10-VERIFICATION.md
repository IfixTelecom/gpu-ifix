...
gaps_closed_phase_10_2026_05_26:
  ...
  deferred_gaps:
    s9_per_tenant_smoke_6_tenants: PASS_PARTIAL 2026-05-26T20:59Z — 6 tenants provisioned (4 via scripts/integration-smoke/provision-tenants.sh: telefonia/sensitive, cobrancas/sensitive, campanhas/normal, voice-api/normal; 2 manual via gatewayctl: converseai/normal, chat-ifix/normal). 5/6 full PASS via smoke scripts against prod URL — converseai (4/4 INT-01 gates: chat+stream+tool+embed), campanhas (4/4 INT-02 gates, latency_ratio 0.293), voice-api (4/4 INT-01 gates). chat-ifix 3/4 PASS (transcription+quality OK; latency_ratio 1.93× baseline — known variance when STT runs on cloud OpenAI Whisper instead of local pod, primary asleep). Sensitive smokes (telefonia/cobrancas) ran via manual curl probe instead of smoke-sensitive-failover.py — script's --induce-failure-via=operator-prestep polls /v1/health/upstreams for state=open but natural breaker FSM oscillates open↔half-open every ~40s on a dead UPSTREAM_LLM_URL, causing race conditions with the 30s polling budget. Manual probes confirmed RES-08 invariant: both sensitive tenants returned HTTP 503 with error.code=upstream_unavailable_for_sensitive_tenant + retry-after=30 (RIDs 019e6614-d551 + 019e6614-e6b5). audit_log shows status_code=503 + route=/v1/chat/completions for both. Phase 11 candidate: extend smoke-sensitive-failover.py to accept FORCED_OPEN as equivalent to natural open, OR make gatewayctl mode actually work.
    s10_rollback_drill_timed: PASS 2026-05-26T22:07Z — manufactured rollback drill using develop-1311a25 (the PR #6 merge commit, immediately pre-hotfix) as the previous-tag target. Direction 1 (latest-dev → develop-1311a25): wall time 3s, /health 200 + version stamp develop-1311a25. Direction 2 (develop-1311a25 → latest-dev): wall time 3s, /health 200 + version stamp develop-5bd79d1. Both directions well under the 5min INT-06 budget. Side-evidence: a chat probe on the rollback target reproduced the audit-flush 0x8b error (SQLSTATE 22021) + missing audit_log row (RID 019e6653-fc54), confirming the develop→latest hotfix is the actual fix and not a flake. Roll-forward probe restored audit_log + billing_events rows (RID 019e6654-bd5f, upstream=openrouter-chat, tokens_out=3).
...
| S9 6-tenant smoke | PASS_PARTIAL | 5/6 full PASS (converseai/campanhas/voice-api 4/4 gates; telefonia+cobrancas 503 sensitive_block via manual probe); 1/6 partial (chat-ifix 3/4: STT cloud-latency variance) |
| S10 rollback drill timed | PASS | manufactured drill: latest-dev↔develop-1311a25 both 3s wall, /health 200 each direction; bug regression on rollback target confirmed fix is real |
...
3. ~~**S9 per-tenant smoke**~~ — **PASS_PARTIAL 2026-05-26T20:59Z**. 6 tenants provisioned, 5/6 full smoke PASS, sensitive tenants verified via manual probe (smoke-sensitive-failover.py race condition on natural breaker FSM half-open oscillation — Phase 11 candidate). See `gaps_closed_phase_10_2026_05_26.deferred_gaps.s9_per_tenant_smoke_6_tenants` for evidence.
4. ~~**S10 rollback drill timed**~~ — **PASS 2026-05-26T22:07Z** (manufactured drill using develop-1311a25 as previous tag). See `gaps_closed_phase_10_2026_05_26.deferred_gaps.s10_rollback_drill_timed`.
