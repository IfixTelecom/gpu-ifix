...
| - | -------------------------------------- | ------- | ------------------------------------------------------- | ----------------------------------------------------------------------------- |
| 1 | `VAST_PRICE_CAP_DPH`                   | `0.40`  | __                                                      | RES Pitfall 5 — epsilon comparison `cap+0.0001` (D-A2)                        |
| 2 | `MONTHLY_EMERGENCY_BUDGET_BRL`         | `200`   | __                                                      | D-D2 — Sentry WARNING only when crossed; **does not** auto-block provisioning |
| 3 | `USD_TO_BRL_RATE`                      | `5.0`   | __                                                      | D-D4 — operator updates quarterly for cost audit reports                      |
| 4 | `EMERGENCY_POD_IMAGE_TAG`              | `v1.0`  | __                                                      | Phase 1 publishes both `:v1.0` and `:latest`; pin for reproducibility         |
| 5 | `PRIMARY_HOST_ID`                      | `0`     | __                                                      | D-A2 — `host_id != PRIMARY_HOST_ID` filter applied only when known (≠0)       |
...
| # | Env var                                  | Default | Decision (`default-aceito`\|`override:VAL`\|`defer:NN`) | Notas                                                       |
| - | ---------------------------------------- | ------- | ------------------------------------------------------- | ----------------------------------------------------------- |
| 6 | `PROVISION_TRIGGER_FAILED_OVER_SECONDS`  | `120`   | __                                                      | D-C1 — bate SC-1 example "e.g., 2 min"; pode encurtar (60s) sob outage crítico |
| 7 | `PROVISION_COLDSTART_BUDGET_SECONDS`     | `600`   | __                                                      | D-A4 — bate SC-1 literal "≤10min once /health passes"       |

---

## 2. `VAST_AI_API_KEY` confirmation checklist

- [ ] **GitHub Secret present:** `gh secret list -R IfixTelecom/gpu-ifix | grep VAST_AI_API_KEY` returns a row (already added 2026-05-12 per CLAUDE.md token store, but confirm not rotated/expired).
- [ ] **Portainer stack env var planned:** confirm operator will add `VAST_AI_API_KEY` to the `ai-gateway-dev` Portainer stack env vars **before** running plan 06-11 LIVE UAT.
- [ ] **Optional rotation:** if the key was shared in a Claude Code session transcript outside this host, regenerate via Vast.ai → API Keys and update both the GitHub Secret and CLAUDE.md token store before proceeding.
