...
Phase: 06.6 (primary-pod-refactor-strategy-b-full-stack-upstream-images-i) — PARTIAL (LLM scenario GREEN attempt 10, STT/Embed deferred to attempt 11 with pip pin fix in CI)
Plan: 11 of 13 (Task 3 Live UAT IN PROGRESS — 10 attempts, attempt 10 = first pod success, LLM functional, STT/Embed import errors fixed in commit ddb7f44 awaiting CI rebuild)
Next autonomous-eligible work: Phase 07 (Observability). Phase 6.6 attempt 11 + Scenarios 2-6 deferred to next session. CRITICAL: implement BestEffortDestroy 429-retry before next attempt (orphan pod risk = ~$0.60/h silent burn).
...
- **Phase 6.6 (2026-05-18, PARTIAL):** Primary-Pod Refactor — Strategy B Full-Stack Upstream Images. 11/13 plans GREEN on develop (02 vastutil, 03 config +24 PRIMARY_*, 04 custom Dockerfile+supervisord+GHA, 05 schedule+FSM, 06a reconciler core, 06b cross-pkg adapters, 07 migration 0023+sqlc+redisx, 08 main.go wire+emerg subscribe Pitfall #11, 09 gatewayctl primary CLI, 10 9 integration tests, 11 docs partial). Plan 11 Task 3 Live UAT BLOCKED. Plan 12 cleanup BLOCKED until UAT GREEN. CI pipeline GREEN (unit + integration + build-gateway + build-primary-pod).
  ...
  - **Tech debt CRÍTICO discovered:** `BestEffortDestroy` does not retry on HTTP 429 from Vast.ai — leaves pod orphan + burning $$$. Fix needed in `gateway/internal/vastutil/helpers.go` BestEffortDestroy: add exponential backoff retry on 429 + escalate to logged alert if all retries exhausted. Until fixed, every UAT attempt risks orphan pod.
  - **Tech debt:** UAT runbook lacks step "verify no live instances post-force-down via `curl /api/v0/instances/`" — operator should add as Sign-off section.
  - **Next session (deferred):** (a) wait CI run 26064374963 build-primary-pod with pip pin fix; (b) redeploy gateway (env vars already set); (c) UAT attempt 11 — test if STT + Embed boot clean post pip fix; (d) if 4 services GREEN → run Scenarios 2-6 (chat tools, audio, embed, schedule drain, emerg coexist Pitfall #11); (e) BEFORE next attempt: implement BestEffortDestroy 429 retry to avoid orphan spend.
